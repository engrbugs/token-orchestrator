...
一款桌面小部件，实时显示各种 AI 编程工具（包含 Claude Code、Codex、Hermes Agent、OpenCode、OpenClaw、Cursor、Antigravity、Cline、Kimi、Qwen、Grok Build、GitHub Copilot 等）的 Token 用量与 AI 工具用量上限，具备实时多设备同步与历史使用趋势功能，并支持按工具、设备、模型或 session 分项显示。
...
- **实时 Token 追踪**：覆盖 Claude Code、Codex、Hermes Agent、OpenCode、OpenClaw、Cursor、Antigravity、Cline、Kimi、Qwen、Grok Build、GitHub Copilot、Pi、Zed、Kilo Code、MiMo Code、ZCode、Kiro（每轮对话后 UI 在数秒内刷新）
...
- **AI 工具用量上限检测**：支持 Claude Code、Codex、Cursor、Antigravity、OpenCode、Grok、Minimax、GitHub Copilot 与 Kiro，涵盖 session、每周、账单与 credits 窗口，以及 DeepSeek 预付余额与今日/本月消费
...
- **菜单栏（macOS）与系统托盘（Windows）弹出窗口**：图标旁可显示成本、token 数，或 Claude／Codex／Cursor／Antigravity／OpenCode／Grok／Minimax／GitHub Copilot／Kiro 最接近用完的用量上限百分比
...
- **AI 工具用量上限**——选择 Claude Code、Codex、Cursor、Antigravity、OpenCode、DeepSeek、Grok、Minimax、GitHub Copilot 与 Kiro 的用量上限检测与刷新频率。
- **趋势**——需手动开启的使用历史；开启后会采集每日历史，并可打开使用仪表板（活跃热力图、连续天数，以及按工具／按模型堆叠的柱状图与 K 线图）。
- **窗口行为**——选择浮在其他 app 上方、普通窗口，或固定在桌面。
- **托盘模式**——切换为 macOS 菜单栏或 Windows 系统托盘的弹出窗口，并选择图标旁显示的内容：成本、今日 token 数、累计 token 数、成本＋token、最接近用完的 Claude／Codex／Cursor／Antigravity／OpenCode／Grok／Minimax／GitHub Copilot／Kiro 用量上限百分比，或仅显示图标。
...
TOKEN_MONITOR_LIMIT_PROVIDERS=       # 可选——默认为所有支持的提供方（claude、codex、cursor、antigravity、opencode、deepseek、minimax、grok、copilot、kiro）
```
...
- 按客户端与模型的分项统计
- 启用 AI 工具用量上限时，归一化后的 Claude Code／Codex／Cursor／Antigravity／OpenCode／Grok／Minimax／GitHub Copilot／Kiro 用量状态
