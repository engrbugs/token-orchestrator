...
(function exposeTrayComposer(root, factory) {
  ...
  function createTrayComposer(options) {
    ...
    function styleTitle(style) {
      const fallback = {
        appIcon: 'Token Orchestrator icon',
    ...
    function barIconEditor(item) {
      ...
      const choices = [{
        value: 'app',
        label: l('trayComposer.icon.app', 'Token Orchestrator'),
        icon: app?.icon
      }, choiceForRow(0)];
    ...
    function renderItemPopover(id) {
      ...
      if (item.type === 'icon') {
        const iconTypes = [
          { value: 'app', label: l('trayComposer.icon.app', 'Token Orchestrator'), icon: providerChoices('auto').find((choice) => choice.value === 'auto')?.icon },
