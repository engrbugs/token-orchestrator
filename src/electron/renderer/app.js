...
function homeToolSourceRows(period) {
  const enabled = enabledClientSet();
  return Object.entries(period?.clients || {}).filter(([client]) => enabled.has(client)).map(([client, value]) => ({
    key: client,
