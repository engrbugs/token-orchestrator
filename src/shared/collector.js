...
// Antigravity/agy writes native conversation data here. These are source roots,
// not tokscale's generated cache, so watching them gives the collector the same
// near-real-time updates as the IDE itself without creating a sync loop.
function antigravitySourceDirs(home = os.homedir()) {
  return ANTIGRAVITY_DATA_ROOTS.flatMap((name) => {
    const root = path.join(home, '.gemini', name);
    return [path.join(root, 'conversations'), path.join(root, 'brain')];
  });
}
...
function watchClientRootsForClients(clientsCsv) {
  ...
  // antigravity is self-synced (its tokscale cache is written by our sync and must
  // stay watch-excluded), but its native conversation/brain roots and CLI data
  // dir are safe to watch. A source event triggers sync, then the targeted tick;
  // tokscaleClientFilter pulls both Antigravity source ids into that scan.
  const enabled = new Set(String(clientsCsv || '').split(',').map((value) => value.trim().toLowerCase()).filter(Boolean));
  if (enabled.has('antigravity')) {
    const sourceDirs = antigravitySourceDirs().filter(dirExists);
    const antigravityCliDir = antigravityCliDataDir();
    if (dirExists(antigravityCliDir)) sourceDirs.push(antigravityCliDir);
    if (sourceDirs.length > 0) rootsByClient.antigravity = [...new Set(sourceDirs)];
  }
...
function antigravitySourcePath(filePath) {
  if (!filePath) return false;
  const resolved = path.resolve(filePath);
  return antigravitySourceDirs().some((root) => {
    const resolvedRoot = path.resolve(root);
    return resolved === resolvedRoot || resolved.startsWith(resolvedRoot + path.sep);
  });
}
...
function clientDataDirPresence(clientsCsv) {
  ...
  // The cache candidate is generated by sync, so also count native Antigravity
  // sources and CLI data. This keeps a source-only install in `waiting` instead
  // of incorrectly reporting `missing` before its first usage row is parsed.
  if (Object.prototype.hasOwnProperty.call(presence, 'antigravity')) {
    presence.antigravity = presence.antigravity
      || antigravitySourceDirs().some(dirExists)
      || dirExists(antigravityCliDataDir());
  }
  return presence;
}
...
function startCollector(options) {
  ...
  function scheduleTick(reason, eventClients, forceSelfSync = null) {
    if (stopped) return;
    ...
    debounceTimer = setTimeout(() => {
      ...
      if (tickInFlight) { scheduleTick(reason, eventClients, forceSelfSync); return; }
      runTick(reason, {
        todayOnly: true,
        targetClients: takeWatchClients(),
        ...(forceSelfSync ? { forceSelfSync } : {})
      });
    }, watchDebounceMs);
  }
      ...
      watcher.on('all', (event, filePath) => {
        ...
        if (watchTriggersCollection) {
          scheduleTick(
            ...
            eventClients,
            antigravitySourcePath(filePath) ? ['antigravity'] : null
          );
