...
`windows[].kind` is `session`, `weekly`, or `billing`. `windows[].metric` is an optional stable machine-readable role; `credits` identifies a provider's balance/credits meter independently of its display label (currently OpenRouter account credits and third-party account/token quota). `windows[].detail` is an optional bounded display-only description for a window, such as the Kimi-vs-Code composition of the single shared monthly membership meter; it must not contain credentials or raw provider response data.
