...
アクティビティヒートマップとトレンドダッシュボードは、各ツールがディスクに残しているセッションファイルから構築されます。**Claude Code はデフォルトで 30 日を過ぎたトランスクリプトを削除します**（`cleanupPeriodDays`）。そのため、それより古い日付における Claude Code の分だけがこれらの表示から抜け落ちます。Claude Code だけを使った日は空白になり、他のツールも使った日はその分が残ります。今日/今月/全期間の合計への影響は小さく、**削除されたセッション使用量を保持**（設定 → 収集）がツールに削除されたセッションの期間合計を保ちますが、ヒートマップが描く日次履歴までは復元できません。
